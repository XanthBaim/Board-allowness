$("document").ready(function () {
  ...
  function skipToEnd(message) {
    ...
    const error =
      currentLanguage === "greek"
        ? "Λυπούμαστε αλλά δεν δικαιούστε το επίδομα σίτισης σπουδαστών/στριών των Ακαδημιών Εμπορικού Ναυτικού (ΑΕΝ)"
  ...
  function submitForm() {
    ...
    currentLanguage === "greek"
      ? $(".question-container").append(
          "<br /><br /><h5 class='answer'>Τα δικαιολογητικά που πρέπει να προσκομίσετε για να λάβετε το επίδομα σίτισης είναι τα εξής:</h5><br />"
        )
      : $(".question-container").append(
  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
      ...
      if (currentQuestion === 0 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Δεν μπορείτε να κάνετε αίτηση για αυτό το επίδομα.") : skipToEnd("You are not eligible to apply for this allowance.");
      } else if (currentQuestion === 1 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Δεν τηρείτε τις βασικές προϋποθέσεις για να υποβάλετε αίτηση για αυτό το επίδομα.") : skipToEnd("You do not meet the basic requirements to apply for this allowance.");
      } else if (currentQuestion === 2 && (selectedRadioButtonIndex === 2 || selectedRadioButtonIndex === 1) ) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Δεν τηρείτε τις βασικές προϋποθέσεις για να υποβάλετε αίτηση για αυτό το επίδομα.") : skipToEnd("You do not meet the basic requirements to apply for this allowance.");
      } else if (currentQuestion === 3 && selectedRadioButtonIndex === 1) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Δεν μπορείτε να πάρετε το επίδομα ξανά για το ίδιο ακαδημαϊκό έτος. ") : skipToEnd("You cannot receive the allowance again for the same academic year.");
      } else if (currentQuestion === 5 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Δεν θεωρείσαι πλέον φοιτητής. Άρα δεν μπορείς να λάβεις το επίδομα. ") : skipToEnd("You are no longer considered a student, so you cannot receive the allowance.");
      } else if (currentQuestion === 8 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Δεν τηρείς τα εισοδηματικά κριτήρια. ") : skipToEnd("You do not meet the income criteria.");
        ...
        // Προσδιορισμός της επόμενης ερώτησης βάσει λογικής
        let nextIndex;

// Παίρνουμε το αντικείμενο της τρέχουσας ερώτησης
        let currentQ = currentLanguage === "greek" ? all_questions[currentQuestion] : all_questions_en[currentQuestion];

// Έλεγχος αν έχει δηλωθεί custom ροή
        if (currentQ.next && currentQ.next[selectedRadioButtonIndex - 1] !== undefined) {
        nextIndex = currentQ.next[selectedRadioButtonIndex - 1];
        } else {
          nextIndex = currentQuestion + 1; // default linear flow
        }

// Αν δεν υπάρχουν άλλες ερωτήσεις, κάνε submit
        if (nextIndex === "submit") {
          submitForm();
        } else {
